...
sub post_transaction {
    ...
    my %defaults = $form->get_defaults( $dbh,
        \@{ [ 'fxgainloss_accno_id', 'cdt', 'precision' ] } );
    ...
    foreach $accno (@taxaccounts) {
        ...
        $tax{amount}{$accno} =
          $form->round_amount( $amount - $diff, $form->{precision} );
    ...
    for ( 1 .. $form->{rowcount} ) {
        ...
        $form->{"linetaxamount_$_"} =
          $form->parse_amount( $myconfig, $form->{"linetaxamount_$_"} );

        # set $form->{linetax} = 1 if linetax is enabled
        $form->{linetax} = ( $form->{"linetaxamount_$_"} > 0 )
          if !$form->{linetax};
    }
        ...
        if ( $amount{fxamount}{$i} || $form->{"linetaxamount_$i"} ) {
            ...
            if ( $form->{taxincluded} ) {
                $amount =
                  ...
                  : 0;
                $taxamount =
                  $form->round_amount( $amount - $fxdiff, $form->{precision} );
                $amount{fxamount}{$i} -= $taxamount;
                $fxdiff =
                  $form->round_amount( $taxamount - ( $amount - $fxdiff ), 10 );
            }
            ...
            my ($tax_chart_id) = $dbh->selectrow_array(
    ...
    for $i ( 1 .. $form->{paidaccounts} ) {
        ...
        if ($fxamount) {
            ...
            $amount =
              $form->round_amount( $paidamount - $diff, $form->{precision} );
    ...
    $paid =
      ( $fxinvamount == $paid )
      ? $invamount
      : $form->round_amount( $paid * $form->{exchangerate},
        $form->{precision} );
    ...
    if ( $table eq 'ap' ) {
        ...
        $form->{executiondate} = undef;
    }
    my $executiondate =
      ( $form->{executiondate} ) ? qq|'$form->{executiondate}'| : 'NULL';
    ...
    if ( $form->{vc} eq 'customer' ) {
        ...
        ( $form->{membernumber}, $form->{dcn}, $form->{rvc} ) =
    ...
    # add voucher
    if ( $form->{batchid} ) {
        $form->{voucher}{transaction}{vouchernumber} =
          $form->update_defaults( $myconfig, 'vouchernumber', $dbh )
          unless $form->{voucher}{transaction}{vouchernumber};
    ...
    for $i ( 1 .. $form->{paidaccounts} ) {
        ...
        if ( $paid{fxamount}{$i} ) {
		    ...
		    $paymentmethod_id)|;
                ...
                if ( $form->{currency} ne $form->{defaultcurrency} ) {
                    ...
                    $amount = $form->round_amount(
                        (
                            $form->round_amount(
                                $paid{fxamount}{$i} * $form->{exchangerate},
			...
			$cleared, '$approved', $voucherid)|;
                    ...
                    $amount = $form->round_amount(
                        $paid{amount}{$i} - $paid{fxamount}{$i} + $amount,
                        $form->{precision} );
                    if ($amount) {
                        $query =
                          qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
...
sub delete_transaction {
    ...
    my $id;
    for $id (qw(id glid)) {
        for (qw(acc_trans dpt_trans payment status pay_trans)) {
            if ( $form->{$id} ) {
                $query = qq|DELETE FROM $_ WHERE trans_id = $form->{$id}|;
                $dbh->do($query) || $form->dberror($query);
            }
...
sub delete_transactions {
    ...
    my $query = qq|SELECT * FROM | . lc $form->{ARAP} . qq| WHERE id = ?|;
    my $sth   = $dbh->prepare($query) || $form->dberror;
            ...
            if ( $ref->{id} ) {
                ...
                if ( $ref->{invoice} ) {
                    ...
                    if ( $form->{ARAP} eq 'AR' ) {
                        ...
                        $deleted =
                          IR->delete_invoice( $myconfig, $form, $spool, $dbh );
                    }
                }
                else {
                    $deleted = AA->delete_transaction( $myconfig, $form, $dbh );
                }

                if ( !$deleted ) {
                    $form->{notdeleted} .= "$ref->{invnumber}\n";
                    $sth->finish;
                    $dbh->disconnect;
                    return 0;
                }
...
sub transactions {
    ...
    my %defaults =
      $form->get_defaults( $dbh, \@{ [ 'precision', 'company', 'revtrans' ] } );
    for ( keys %defaults ) { $form->{$_} = $defaults{$_} }

    $form->{defaultcurrency} =
      substr( $form->get_currencies( $myconfig, $dbh ), 0, 3 );
    ...
    if ( $form->{outstanding} ) {
        $paid = qq|SELECT SUM(ac.amount) * -1 * $ml
               ...
               AND (c.link LIKE '%${ARAP}_paid%'
                    ...
                    OR c.link = '')|;
        $paid .= qq|
               AND ac.transdate <= '$form->{transdateto}'|
          if $form->{transdateto};
        $form->{summary} = 1;
        $form->{l_memo}  = "";
    }
...
|;
    ...
    $where .= " AND a.created >= '$form->{createdfrom}'"
      if $form->{createdfrom};
    $where .= " AND a.created < (DATE '$form->{createdto}' + INTERVAL '1 day')"
      if $form->{createdto};

    $where .= " AND a.updated >= '$form->{updatedfrom}'"
      if $form->{updatedfrom};
    $where .= " AND a.updated < (DATE '$form->{updatedto}' + INTERVAL '1 day')"
      if $form->{updatedto};
  ...
  # In case we're filtering for outstanding transactions, skip those fully paid.
        if ( $form->{outstanding} ) {
            next
              if $form->round_amount( $ref->{amount}, $form->{precision} ) ==
              $form->round_amount( $ref->{paid}, $form->{precision} );
        }
    ...
    delete $curr{ $form->{defaultcurrency} };
...
sub get_name {
    ...
    if ( $myconfig->{dbdriver} eq 'DB2' ) {
        ...
        $duedate =
          ( $form->{transdate} )
          ? "to_date('$form->{transdate}', '$dateformat') + c.terms"
    ...
    my $shipto     = ", s.*" if !$form->{linkshipto};
    ...
    $ref->{arap_accno} .= qq|--|;
    $ref->{arap_accno} .=
      ( $ref->{arap_accno_translation} )
      ? $ref->{arap_accno_translation}
      : $ref->{arap_accno_description};
    $ref->{payment_accno} .= qq|--|;
    $ref->{payment_accno} .=
      ( $ref->{payment_accno_translation} )
      ? $ref->{payment_accno_translation}
      : $ref->{payment_accno_description};
    $ref->{discount_accno} .= qq|--|;
    $ref->{discount_accno} .=
      ( $ref->{discount_accno_translation} )
      ? $ref->{discount_accno_translation}
      : $ref->{discount_accno_description};
    ...
    $form->{currency} =
      ( $form->{currency} ) ? $form->{currency} : $form->{defaultcurrency};
    if ( $form->{transdate}
        && ( $form->{currency} ne $form->{defaultcurrency} ) )
    {
        $form->{exchangerate} ||=
          $form->get_exchangerate( $myconfig, $dbh, $form->{currency},
            $form->{transdate} );
    }
    ...
    if ( $form->{transdate} ) {
        $where =
          qq|AND (t.validto >= '$form->{transdate}' OR t.validto IS NULL)|;
    }
    else {
...
sub company_details {
    ...
    my $query =
      qq|SELECT ct.$form->{vc}number, ct.name, ad.address1, ad.address2,
    ...
    $ref = $sth->fetchrow_hashref(NAME_lc);
    for (
        qw(firstname lastname salutation contacttitle occupation typeofcontact gender)
      )
    {
        $form->{$_} = $ref->{$_};
    }
    ...
    my ( undef, $id ) = split /--/, $form->{employee};
    $id *= 1;
    $query = qq|SELECT workphone, workfax, workmobile
              FROM employee
              WHERE id = $id|;
    ( $form->{workphone}, $form->{workfax}, $form->{workmobile} ) =
      $dbh->selectrow_array($query);

    my @df =
      qw(weightunit cdt company companyemail companywebsite address tel fax businessnumber annualinterest latepaymentfee restockingcharge);
    ...
    # banking details
    if ( $form->{vc} eq 'customer' ) {
        my ($paymentaccno) = split /--/,
          $form->{"AR_paid_$form->{paidaccounts}"};
        ...
        for (qw(iban bic membernumber clearingnumber rvc dcn)) {
            $form->{$_} = $form->{"bank$_"};
        }
    }
