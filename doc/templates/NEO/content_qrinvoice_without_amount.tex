%%%%%%%%%%%%%  TITLE  %%%%%%%%%%%%%
% Typ: QR invoice / QR Rechnung
% Name: content-qrinvoice.tex
% Creation Date: 14. November 2022
...
\AddToShipoutPicture{\BackgroundWithoutFooter} % Required if a background with a logo should be displayed
    ...
    %%%%% PAYMENT PART
        ...
        % Swiss QR Code section
            \node (PSQRCS) at ({\QRrwid+\QRpad},{\QRphei-2.34*\QRpad+5mm}) {
                \begin{minipage}{\linewidth}\raggedright
\ifthenelse{\equal{<%customeremail%>}{}}{\unicodeqrcode{SPC\\0200\\1\\<%qribanqr%>\\K\\<%companyqr%>\\<%companyaddress1qr%>\\<%companyzipqr%> <%companycityqr%>\\\\\\CH\\\\\\\\\\\\\\\\\\<%currency%>\\K\\<%nameqr%>\\<%address1qr%>\\<%zipcodeqr%> <%cityqr%>\\\\\\CH\\QRR\\<%dcn%>\\<%invdescriptionqr%>\\EPD\\<%strdbkginfqr%>\\\\}}{\unicodeqrcode{SPC\\0200\\1\\<%qribanqr%>\\K\\<%companyqr%>\\<%companyaddress1qr%>\\<%companyzipqr%> <%companycityqr%>\\\\\\CH\\\\\\\\\\\\\\\\\\<%currency%>\\K\\<%nameqr%>\\<%address1qr%>\\<%zipcodeqr%> <%cityqr%>\\\\\\CH\\QRR\\<%dcn%>\\<%invdescriptionqr%>\\EPD\\<%strdbkginfqr%>\\eBill/B/<%customeremail%>\\}}
