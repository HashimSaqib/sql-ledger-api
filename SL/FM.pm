...
sub process_dropbox {
    ...
    my $token_url        = 'https://api.dropbox.com/oauth2/token';
    my $shared_link_url =
...
qq/{"path": "$dbx_path", "mode": "add", "autorename": true, "mute": false}/,
            ...
            if ( $status_code == 401 && $attempt == 1 && $refresh_token ) {
                my $refresh_successful =
                  _refresh_dropbox_token( $dbs, $c, $connection, $client_id,
                    $client_secret, \$access_token );

                if ( !$refresh_successful ) {
                    $final_result = { error => "Dropbox token refresh failed" };
                    last;
                }
                ...
                $c->app->log->error(
"Dropbox upload failed: Status $status_code - $error_message "
...
sub process_google_drive {
    ...
    my $parent_id   = _ensure_google_drive_folders(
...
"Failed to create/find Google Drive folder structure for: $folder_path"
    ...
    foreach my $file_info ( @{ $data->{files_info} } ) {
        ...
        while ( $attempt <= MAX_UPLOAD_ATTEMPTS && !$upload_success ) {
            ...
            if ( $status_code == 401 && $attempt == 1 && $refresh_token ) {
                my $refresh_successful =
                  _refresh_google_drive_token( $dbs, $c, $connection,
                    $client_id, $client_secret, \$access_token );

                if ( !$refresh_successful ) {
                    $c->app->log->error("Google Drive token refresh failed");
                    last;
                }

                # Re-get parent folder ID with new token
                $parent_id = _ensure_google_drive_folders(
                    $ua,        $access_token,  $refresh_token,
                    $client_id, $client_secret, $dbs,
                    $c,         $connection,    $folder_path
                );
                unless ($parent_id) {
                    $c->app->log->error(
                        "Failed to find/create folders with refreshed token");
                    last;
                }
                ...
                $c->app->log->error(
"Google Drive upload failed: Status $status_code - $error_message "
                      . "(Connection type: $connection->{type})" );
...
sub _refresh_google_drive_token {
    my ( $dbs, $c, $connection, $client_id, $client_secret, $access_token_ref )
      = @_;
    my $ua            = Mojo::UserAgent->new;
    my $token_url     = 'https://oauth2.googleapis.com/token';
    my $refresh_token = $connection->{refresh_token};
    ...
    unless ($refresh_token) {
        $c->app->log->error(
            "Missing refresh token for Google Drive connection");
        _update_connection_status( $dbs, $connection->{id}, 'error',
    ...
    unless ( $refresh_res->is_success ) {
        _update_connection_status( $dbs, $connection->{id}, 'error',
            "Token refresh failed: "
              . ( $refresh_res->message || "Unknown Refresh Error" ) );
        return 0;
    }
    ...
    my $expiry_time          = Time::Piece->gmtime() + int($expires_in);
    my $expiry_timestamp_str = $expiry_time->strftime('%Y-%m-%d %H:%M:%S+00');

    my $update_ok =
      _update_connection_status( $dbs, $connection->{id}, 'active',
        undef, $new_access_token, $expiry_timestamp_str );
    ...
    # Update the token reference so caller has the new token
    $$access_token_ref = $new_access_token;
    return 1;
}

sub _refresh_dropbox_token {
    my ( $dbs, $c, $connection, $client_id, $client_secret, $access_token_ref )
      = @_;
    my $ua            = Mojo::UserAgent->new;
    my $token_url     = 'https://api.dropbox.com/oauth2/token';
    ...
    unless ($refresh_token) {
        $c->app->log->error("Missing refresh token for Dropbox connection");
        _update_connection_status( $dbs, $connection->{id}, 'error',
            "Missing refresh token" );
        return 0;
    }
...
sub _ensure_google_drive_folders {
    ...
    # Split the path into components - remove leading slash and empty parts
    my @folders = grep { $_ } split( '/', $folder_path );
    return 'root' unless @folders;    # If no folders, use root
...
"name='$folder_name' and mimeType='application/vnd.google-apps.folder' and '$parent_id' in parents and trashed=false";
        my $encoded_query = Mojo::Util::url_escape($search_query);

        my $tx = $ua->get(
            "$folder_url?q=$encoded_query" => {
                'Authorization' => "Bearer $access_token"
            }
        );
...
sub _create_google_drive_public_link {
    ...
    # Whether or not permission creation succeeded, try to get a web view link
    my $file_url =
    ...
    # If we can't get a webViewLink, return a direct link format
...
sub insert_file_record {
    ...
    my $sql =
"INSERT INTO files (module, name, extension, location, path, link, reference_id) "
      . "VALUES (?, ?, ?, ?, ?, ?, ?)";
...
sub _update_connection_status {
    my (
        $dbs,           $connection_id, $status,
        $error_message, $new_token,     $expiry_str
    ) = @_;
    ...
    if ( defined $new_token && defined $expiry_str ) {
        $sql .= ", access_token = ?, token_expires = ?";
        push @params, $new_token, $expiry_str;
    }
...
sub delete_files {
    ...
    # Validate and retrieve the reference ID
    ...
    # Loop over each file record and attempt deletion
...
"SELECT * FROM connections WHERE status = 'active' AND type = 'dropbox' LIMIT 1"
            ...
            # First, extract file_id from path or link
            my $file_id;
           ...
           # If we failed to extract from link, check if path is actually the ID
            elsif ( $path && $path =~ /\/([^\/]+)$/ ) {
                $file_id = $1;
            }
...
sub delete_file {
    ...
    # Validate the presence of a filename
    ...
    if ( $location eq 'local' ) {
        ...
        if ( -e $path ) {
            ...
            $c->app->log->warn(
                "Local file $path does not exist, skipping unlink.");
            $deleted = 1;    # Consider it deleted even if file doesn't exist
        }
    }
    elsif ( $location eq 'dropbox' ) {
...
"SELECT * FROM connections WHERE status = 'active' AND type = 'dropbox' LIMIT 1"
        ...
        # Extract file ID from path or link
        my $file_id;
        ...
        # If that fails, try to extract from the path
        elsif ( $path && $path =~ /\/([^\/]+)$/ ) {
            $file_id = $1;
        }
...
"SELECT * FROM connections WHERE status = 'active' AND type = 'google_drive' LIMIT 1"
    ...
    # If deletion was successful, remove database record
    if ($deleted) {
        my $sth = $dbs->query( "DELETE FROM files WHERE id = ?", $file->{id} );
        unless ( $sth && $sth->rows > 0 ) {
            $c->app->log->error(
