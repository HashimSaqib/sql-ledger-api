...
sub post_transaction {
  ...
  my $executiondate = ($form->{executiondate}) ? qq|'$form->{executiondate}'| : 'NULL';
  ...
  if (keys %dist_amounts) {
    ...
    while (my ($source) = $sth->fetchrow_array) {
      ...
      my $dist_module = $table eq 'ar' ? 'ar' : 'ap';
      ...
      if ($dist_id) {
        ...
        $sth_insert->execute($source, $form->{id}, $abs_amount, $dist_module) || $form->dberror($query_insert);
      }
    }
    $sth->finish;
  }
...
sub delete_transactions {
  ...
  my $query = qq|SELECT * FROM |. lc $form->{ARAP} .
              qq| WHERE id = ?|;
    ...
    if ($form->{id}) {
      ...
      if ($ref->{id}) {
        ...
        if ($ref->{invoice}) {
          ...
          if ($form->{ARAP} eq 'AR') {
            $deleted = IS->delete_invoice($myconfig, $form, $spool, $dbh);
          } else {
            $deleted = IR->delete_invoice($myconfig, $form, $spool, $dbh);
          }
        } else {
...
sub transactions {
 ...
 if ($form->{outstanding}) {
    $paid = qq|CASE WHEN ROUND(a.amount::numeric, $form->{precision}) = ROUND(a.paid::numeric, $form->{precision}) AND a.paid != 0 AND a.paid != 0 |;
    $paid .= qq|
